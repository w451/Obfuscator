...
void obfuscator::create_functions(std::vector<pdbparser::sym_func>functions) {
	...
	for (auto function : functions) {
		...
		new_function.addobf = function.addobf;
		new_function.leaobf = function.leaobf;
		new_function.antidisassembly = function.antidisassembly;
		new_function.popobf = function.popobf;
		new_function.retobf = function.retobf;
		new_function.xorobf = function.xorobf;
...
bool obfuscator::find_instruction_by_id(int funcid, int instid, instruction_t* inst) {
	...
	if (func == this->functions.end()) {
		printf("Couldn't find funccy\n");
		return false;
	}

	//printf("Found funccy ");
	//printf(func->name.c_str());
	//printf("\n");
	...
	printf("Couldn't find instruccy\n");
	for (instruction_t t : func->instructions) {
		t.print();
		printf("->%d\n",t.inst_id);
	}
	return false;
	...
	for (auto instruction = func->instructions.begin(); instruction != func->instructions.end(); instruction++) {
		...
		if (instruction->isjmpcall && instruction->relative.target_inst_id != -1) {
			...
			if (!this->find_instruction_by_id(instruction->relative.target_func_id, instruction->relative.target_inst_id, &inst)) {
				printf("Looking for instr %d\n", instruction->relative.target_inst_id);
				return false;
			}
...
void obfuscator::run(PIMAGE_SECTION_HEADER* new_section, GlobalArgs args) {
	...
	for (auto func = functions.begin(); func != functions.end(); func++) {
		...
		for (auto instruction = func->instructions.begin(); instruction != func->instructions.end(); instruction++) {			
			...
			//Obfuscate sub rsp, imm / add rsp, imm
			if (func->stackobf) {
				if ((instruction->zyinstr.mnemonic == ZYDIS_MNEMONIC_ADD 
					|| instruction->zyinstr.mnemonic == ZYDIS_MNEMONIC_SUB)
					&& instruction->zyinstr.operands[0].type == ZYDIS_OPERAND_TYPE_REGISTER 
					...
					this->obfuscate_stack(func, instruction);
			}
			...
			//Obfuscate cmp with constant
			if (func->cmpobf) {
				if (instruction->zyinstr.mnemonic == ZYDIS_MNEMONIC_TEST)
				{
					this->convert_test(func, instruction);
				}
				if (instruction->zyinstr.mnemonic == ZYDIS_MNEMONIC_CMP)
				{
					
					this->obfuscate_cmp(func, instruction);
				}
			}

			//Obfuscate POP rax
			if (func->popobf) {
				if (instruction->zyinstr.mnemonic == ZYDIS_MNEMONIC_POP)
				{
					this->obfuscate_pop(func, instruction);
				}
			}


			//Obfuscate xor REG, REG
			if (func->xorobf) {
				if (instruction->zyinstr.mnemonic == ZYDIS_MNEMONIC_XOR)
				{
			...
			//Obfuscate ret
			if (func->retobf) {
				if (instruction->zyinstr.mnemonic == ZYDIS_MNEMONIC_RET)
				{
					this->obfuscate_return(func, instruction);
				}
			}

			//Obfuscate calls
			if (func->callobf) {
				if (instruction->zyinstr.mnemonic == ZYDIS_MNEMONIC_CALL)
				{
					this->obfuscate_call(func, instruction);
				}
			}
	...
	//printf("pre: %d\n",functions.size());
	if (args.outline_funcs) {
		this->outline_instrs(&functions, args.outline_max, args.outline_strlen, args.anti_xref);
	}
	//printf("post: %d\n", functions.size());
...
bool is_stackop(ZydisDecodedInstruction instr)
{
	...
	for (int i = 0; i < ZYDIS_MAX_OPERAND_COUNT; i++) {
		if (instr.operands[i].reg.value == ZYDIS_REGISTER_RSP
			|| instr.operands[i].reg.value == ZYDIS_REGISTER_ESP
			|| instr.operands[i].reg.value == ZYDIS_REGISTER_SP
			|| instr.operands[i].mem.base == ZYDIS_REGISTER_RSP
			|| instr.operands[i].mem.base == ZYDIS_REGISTER_ESP
			|| instr.operands[i].mem.base == ZYDIS_REGISTER_SP) {
			return true;
		}
	} 
...
{
	for (int i = 0; i < ZYDIS_MAX_OPERAND_COUNT; i++) {
		if (instr.operands[i].reg.value == ZYDIS_REGISTER_RIP
			|| instr.operands[i].reg.value == ZYDIS_REGISTER_EIP
			|| instr.operands[i].reg.value == ZYDIS_REGISTER_IP
			|| instr.operands[i].mem.base == ZYDIS_REGISTER_RIP
			|| instr.operands[i].mem.base == ZYDIS_REGISTER_EIP
			|| instr.operands[i].mem.base == ZYDIS_REGISTER_IP) {
			return true;
		}
	}
